...
internal class SwitchPadCurveManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        int i = 0;
        foreach (BleSensorCavybandBase each in BleSensorsManager.Instance.SensorsByBleAddress.Values)
        {
            if (index == i)
            {
                _sensor = each;
    ...
    protected void OnMotionDataReceived(BleSensorCavybandBase a_sensor, Quaternion a_quaternion, Vector3 a_acc)
    {
